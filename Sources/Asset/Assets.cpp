...
void Assets::SaveAsset(AssetRef asset) {
    ...
    std::string ensurePath = (std::filesystem::path("Assets/Mods") / std::filesystem::path(path)).parent_path().generic_string();
    std::filesystem::create_directory(ensurePath);

    std::string relPath = fmt::format("Assets/Mods/{}", path);
