...
void PalleteCache::UploadPallete(PalleteAsset* InPallete) {
    ...
    uint8* color = Get().colors.data() + offset;
    uint8* mat = Get().materials.data() + offset;
    ...
    CmdCopy(Get().colors.data(), Get().ColorTexture, Get().colors.size());
    CmdCopy(Get().materials.data(), Get().MaterialTexture, Get().materials.size());
