...
layout(push_constant) uniform uPushConstant{
    ...
    int _ShadowVoxRID;
    int _SpotLightsBufferRID;
    int LightIndex;
};

BINDING_VIEW_BUFFER()
BINDING_SPOT_LIGHTS_BUFFER()
...
void main() {
    ...
    float shadow = 1.0;
    {
        vec3 wd = normalize(lightDir)*10.0;
        float hitDist = lightDistance*10.0;
        ...
        #else
            if(raycastShadowVolumeSuperSparse(wcp+normal*0.5, wd, hitDist) < hitDist){
                shadow = 0.0;
            }
        #endif
    }
    ...
    float angleDist = max(dot(normalize(lightDir), light.Direction)-(1.0-light.Angle), 0.0)/light.Angle;
    float attenuation = pow(angleDist, light.AngleAttenuation) * clamp(light.Range-lightDistance, 0, 1) / pow(dist, light.Attenuation);
