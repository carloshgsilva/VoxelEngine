...
void ViewportWindow::OnGUI() {
	...
	//Drag Asset Target
	{
		...
		if (asset != Asset::NullGUID) {
			...
			Editor->Cmd(
				...
				[&R, this, data = data]() {
				this->_World->Destroy(data->spawnedEntity);
			},
				"Spawn Prefab");
	...
	if (!windowOpen) {
		Engine::OnBeforeUpdate([=]() {
			Editor->CloseViewport(this);
		});
	}
}

void ViewportWindow::OnUpdate(DeltaTime dt) {
	if (Focused) {
		_Camera->Update(dt);
	}
