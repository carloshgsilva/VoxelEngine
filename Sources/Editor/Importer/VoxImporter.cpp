...
struct VoxTransformMatrix {
    ...
    glm::ivec3 transform(glm::ivec3 vec, glm::ivec3& size) {
        ...
        return glm::ivec3(_sx ? (getFromVec(_rx, size) - x - 1) : x, _sy ? (getFromVec(_ry, size) - y - 1) : y, _sz ? (getFromVec(_rz, size) - z - 1) : z);
    }
    glm::ivec3 transformSize(glm::ivec3& size) {
        return glm::ivec3(getFromVec(_rx, size), getFromVec(_ry, size), getFromVec(_rz, size));
    }
};
...
class VoxGroup : public VoxNode {
   public:
    std::vector<int> childrenIds;
};
class VoxTransform : public VoxNode {
   public:
    int x, y, z, childId;
    VoxTransformMatrix matrix;
    std::string name;
};
class VoxShape : public VoxNode {
   public:
    int shapeId;
};
...
class VoxImportContext {
   public:
    ...
    bool Import(Stream& s) {
        ...
        if (std::memcmp(VOX_, "VOX ", 4)) {
            Log::warn("Trying to load an invalid vox file! {}", s.GetIdentifier());
            return false;
        }
...
static entt::entity CreateEntity(VoxImportContext& ctx, std::shared_ptr<VoxTransform> root) {
    ...
    if (shape != nullptr) {
        ...
        {  // Offset center
            glm::ivec3 center = glm::ivec3(root->matrix._sx ? tSize.x - tSize.x / 2 : tSize.x / 2, root->matrix._sz ? tSize.z - tSize.z / 2 : tSize.z / 2,
        ...
        std::string shapeName = fmt::format("{}", TEMP_counter++);
